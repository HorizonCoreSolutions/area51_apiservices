...
class BypassSingleWagering(TransactionTestCase, WageringTestMixin):
    """Tests for __single_wr_pay function (tested through platform_pay)."""

    @transaction.atomic
    def test_only_bypass_when_one(self):
        """Payment should add to WR balance when active."""
        user = self.create_user(balance=Decimal("10.00"))
        wr = self.create_wagering_requirement(
            user,
        ...
        data = platform_bet(user, Decimal('2.00'))
        user.refresh_from_db()
        wr.refresh_from_db()

        self.assertEqual(user.balance, Decimal("18.00"))
