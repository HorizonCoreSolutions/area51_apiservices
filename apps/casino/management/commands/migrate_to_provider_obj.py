...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        for game in CasinoGameList.objects.all():
            ...
            provider = self.fetch_provider_obj(vendor_name, game)
    ...
    def fetch_provider_obj(self, name: str, game: CasinoGameList) -> Providers:
        queryset = Providers.objects.filter(name=name, admin=game.casino_management.admin)

        if queryset.exists():
            return queryset.first()

        return Providers.objects.create(name=name, admin=game.casino_management.admin)